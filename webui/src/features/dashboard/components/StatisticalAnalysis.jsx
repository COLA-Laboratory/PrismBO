...
import { Button, Modal, Form, Input, Select, Table, Space, Tag, Divider, Popconfirm, message } from "antd";
      ...
      >
        ...
        >
          <Form layout="vertical">
            ...
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}><Form.Item label="Refiner"><Select value={modalForm.Refiner} options={algorithmData.spaceRefiner.map(v => ({ label: v.name, value: v.name }))} onChange={v => setModalForm(f => ({ ...f, Refiner: v }))} allowClear showSearch /></Form.Item></div>
              <div style={{ flex: 1 }}><Form.Item label="Sampler"><Select value={modalForm.Sampler} options={algorithmData.sampler.map(v => ({ label: v.name, value: v.name }))} onChange={v => setModalForm(f => ({ ...f, Sampler: v }))} allowClear showSearch /></Form.Item></div>
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}><Form.Item label="Pretrain"><Select value={modalForm.Pretrain} options={algorithmData.pretrain.map(v => ({ label: v.name, value: v.name }))} onChange={v => setModalForm(f => ({ ...f, Pretrain: v }))} allowClear showSearch /></Form.Item></div>
              <div style={{ flex: 1 }}><Form.Item label="Model"><Select value={modalForm.Model} options={algorithmData.model.map(v => ({ label: v.name, value: v.name }))} onChange={v => setModalForm(f => ({ ...f, Model: v }))} allowClear showSearch /></Form.Item></div>
            </div>
            <div style={{ display: 'flex', gap: 16 }}>
              <div style={{ flex: 1 }}><Form.Item label="Acquisition Function"><Select value={modalForm.ACF} options={algorithmData.acf.map(v => ({ label: v.name, value: v.name }))} onChange={v => setModalForm(f => ({ ...f, ACF: v }))} allowClear showSearch /></Form.Item></div>
              <div style={{ flex: 1 }}><Form.Item label="Normalizer"><Select value={modalForm.Normalizer} options={algorithmData.normalizer.map(v => ({ label: v.name, value: v.name }))} onChange={v => setModalForm(f => ({ ...f, Normalizer: v }))} allowClear showSearch /></Form.Item></div>
            </div>
          </Form>
        </Modal>
      </Modal>
