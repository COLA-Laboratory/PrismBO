...
from prismbo.datamanager.minhash import MinHasher
...
class LSHCache:
    def __init__(self, hasher, num_bands=10):
        ...
        self.random_planes = np.random.randn(hasher.num_hashes, 1536)
        self.random_planes /= np.linalg.norm(self.random_planes, axis=1, keepdims=True)
    ...
    def add(self, key, vector):
        ...
        #     combined_fp.extend(self.hasher.fingerprint(dimension))
        embedding = vector[0]
        num_variables = vector[1]
        num_objectives = vector[2]
        projections = self.random_planes.dot(embedding)
        ...
        for band_idx in range(self.num_bands):
            ...
            band_fp = (tuple(bits[start:end]), num_variables, num_objectives)
            self.buckets[band_idx][band_fp].add(key)
