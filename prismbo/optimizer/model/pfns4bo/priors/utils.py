...
def get_batch_to_dataloader(get_batch_method_):
    ...
    class DL(PriorDataLoader):
        ...
        def get_test_batch(self, **kwargs): # does not increase epoch_count
            return self.gbm(**self.get_batch_kwargs, num_steps=self.num_steps, epoch=self.epoch_count, model=self.model if hasattr(self, 'model') else None, **kwargs)

        def __iter__(self):
            assert hasattr(self, 'model'), "Please assign model with `dl.model = ...` before training."
            self.epoch_count += 1
            return iter(self.gbm(**self.get_batch_kwargs, num_steps=self.num_steps, epoch=self.epoch_count - 1, model=self.model) for _ in range(self.num_steps))
