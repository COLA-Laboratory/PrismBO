...
KNOB_PATH = r'./whole_param.json'
RUN_ENGINE_PATH = r'/home/test/debug/PrismBO/external/vector-db-benchmark-master/run_engine.sh'
...
class RealEnv:
    ...
    def get_state(self, knob_vals_arr):
        ...
        for i,record in enumerate(knob_vals_arr):
            ...
            try:
                # result = os.system(f'{RUN_ENGINE_PATH} "" "" glove-100-angular')
                result = sp.run(f'{RUN_ENGINE_PATH} "" "" glove-25-angular', env=env, shell=True, stdout=sp.PIPE)
...
if __name__ == '__main__':
    # print(LHS_sample(5,10))
    real_env = RealEnv()
    print(real_env.default_conf())
    real_env.get_state([real_env.default_conf()])
